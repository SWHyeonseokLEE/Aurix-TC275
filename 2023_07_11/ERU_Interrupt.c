...
#define Encoder_A                     &IfxScu_REQ8_P33_7_IN    /* External request pin                                 */
#define Encoder_B                     &MODULE_P00,0            /* Pin that is toggled by pressing the board's button   */
#define LED                           &MODULE_P00,1            /* LED which gets toggled in Interrupt Service Routine  */
...
void SCUERU_Int0_Handler(void)
{
    ...
    if(IfxPort_getPinState(Encoder_B)==1 && IfxPort_getPinState(LED)==1)
    {
        encoder ++;
    }
    else if(IfxPort_getPinState(Encoder_B)==1 && IfxPort_getPinState(LED)==0)
    {
        encoder -= 1;
    }
}
...
void initPeripheralsAndERU(void)
{
    ...
    IfxPort_setPinMode(Encoder_B, IfxPort_Mode_inputPullUp);              /* Initialize Encoder_B port pin  */
    ...
    /* Trigger pin */
    g_ERUconfig.reqPin = Encoder_A; /* Select external request pin */
