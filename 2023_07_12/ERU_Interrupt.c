...
#define encoder1                        &MODULE_P00,3            /* Pin that is toggled by pressing the board's button   */
#define encoder2                        &MODULE_P00,2            /* LED which gets toggled in Interrupt Service Routine  */
...
void SCUERU_Int0_Handler(void)
{
    ...
    if(rpm_cnt==1)
    {
        time_old = returning()/1000;
    }
    else if(rpm_cnt==240)
    {
        time = returning()/1000;
        rpm = (1 /(time-time_old))*60;
        rpm_cnt=0;
        clear_cnt();
    }
}
...
void initPeripheralsAndERU(void)
{
    ...
    IfxPort_setPinMode(encoder1,  IfxPort_Mode_inputPullDown);              /* Initialize TRIGGER_PIN port pin  */
    IfxPort_setPinMode(encoder2,  IfxPort_Mode_inputPullDown);
