...
float32 fTimer                    = 0;                      // ??
uint16  dutyUpTime                = 0;                      // PWM RISING PULSE
uint16  dutyDownTime              = 0;                      // PWM FALLING PULSE
uint16  cnt2                      = 0;                      // TIMER PULSE 갯수
uint32  g_previousCntVal          = 0;                      // 직전 시간 값
uint32  finalCntVal               = 0;                      // 최종 시간 값
...
void interruptGpt12(void)
{
    ...
    cnt2++;                                                 // TIMER INITERRUPT 발생 시마다 LED가 토글 되므로 LED TOGGLE NUMBER = TIMER NUMBER(기준 타이머)
}
...
{
    uint32 currentCntVal = IfxGpt12_T3_getTimerValue(&MODULE_GPT120);           // TIMER의 현재 값을 받아오는 함수

    if(cnt2 == 0)                                                               // TIMER INTERRUPT의 횟수가 0번일 때는 최종 타이머와 현재 타이머의 값이 같다 (동기화 용)
    {
        /* If no overflow detected */
        finalCntVal = currentCntVal - g_previousCntVal; /* Total counter value calculation */
    }
    else                                                                        // TIMER INTERRUPT의 횟수가 1번 이상일 경우 직전 타이머의 값과 현재 타이머의 합과 같다
    {
         ...
         */
        finalCntVal = (uint32)(currentCntVal - g_previousCntVal) + (cnt2 * MAX_VAL_16_BIT);
    }
    ...
    return finalCntVal;                                                         // 최종 TIMER 값을 RETURN
}
