...
uint8 send_data_1 = 0;
uint8 send_data_2 = 0;
void parsing_rx_buff(void)
{
    if(  (protocal_test_1[0] == 0xb8) && (protocal_test_1[1] == 0xae)  )
    {
        if(protocal_test_1[2] == 'O')
        {
            crc_16_val.check_sum = CRC16_MODBUS(protocal_test_1, 5);
            if(  (crc_16_val.b[1] == protocal_test_1[5]) && (crc_16_val.b[0] == protocal_test_1[6])  )
            {
                send_data_1     = protocal_test_1[3];
                send_data_2     = protocal_test_1[4];
            }
        }
    }
}
...
{
    IfxAsclin_Asc_read(&g_ascHandle, protocal_test_1, &s_count_1, TIME_INFINITE);   /* Transmit data via TX */
    count++;
}
